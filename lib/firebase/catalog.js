...
import {
  ...
  souvenirProducts as fallbackProducts,
} from "../../data/sac-co-do";
...
export async function getPublicStations() {
  ...
  try {
    ...
    return stations.length ? publicStatusFilter(stations) : getFallbackStations();
  } catch (error) {
...
export async function getPublicProducts() {
  ...
  try {
    ...
    return products.length ? publicStatusFilter(products) : getFallbackProducts();
  } catch (error) {
...
export async function getDefaultArCharacter(characterId) {
  ...
  try {
    ...
    return snapshot.docs[0] ? { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } : defaultArCharacter;
  } catch (error) {
