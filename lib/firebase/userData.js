...
const QR_UNLOCK_STORAGE_KEY = "scd_qr_unlocks_v1";

export async function hasStationQrUnlock({ db, uid, stationId }) {
  if (!db || !uid || !stationId) return false;

  const snapshot = await getDoc(doc(db, "users", uid, "journeyQrUnlocks", stationId));
  return snapshot.exists();
}

export async function saveStationQrUnlock({ db, uid, stationId, stationName }) {
  if (!db || !uid || !stationId) return false;

  await setDoc(
    doc(db, "users", uid, "journeyQrUnlocks", stationId),
    {
      stationId,
...
export function hasLocalStationQrUnlock(stationId) {
  if (typeof window === "undefined" || !stationId) return false;

  try {
    const unlockedStations = JSON.parse(window.localStorage.getItem(QR_UNLOCK_STORAGE_KEY) || "[]");
    return Array.isArray(unlockedStations) && unlockedStations.includes(stationId);
  } catch {
    return false;
  }
}

export function saveLocalStationQrUnlock(stationId) {
  if (typeof window === "undefined" || !stationId) return;

  try {
    const unlockedStations = JSON.parse(window.localStorage.getItem(QR_UNLOCK_STORAGE_KEY) || "[]");
    const nextUnlockedStations = Array.isArray(unlockedStations) ? unlockedStations : [];
    if (!nextUnlockedStations.includes(stationId)) {
      window.localStorage.setItem(QR_UNLOCK_STORAGE_KEY, JSON.stringify([...nextUnlockedStations, stationId]));
    }
  } catch {
    window.localStorage.setItem(QR_UNLOCK_STORAGE_KEY, JSON.stringify([stationId]));
  }
}
...
let lastOrderCodeTick = "";
let lastOrderCodeSequence = 0;

export function buildOrderCode(date = new Date()) {
  const tick = String(date.getTime() % 100000000).padStart(8, "0");

  if (tick === lastOrderCodeTick) {
    lastOrderCodeSequence = (lastOrderCodeSequence + 1) % 100;
  } else {
    lastOrderCodeTick = tick;
    lastOrderCodeSequence = 0;
  }

  const sequence = String(lastOrderCodeSequence).padStart(2, "0");
  return `SCD${tick}${sequence}`;
}
...
export async function createOrderFromCart({
  ...
  await addDoc(collection(db, "orders", orderRef.id, "events"), {
    ...
    from: null,
    to: "pending",
    message: paymentMethod === "sepay" ? "Người dùng tạo đơn chờ thanh toán" : "Người dùng tạo đơn COD",
    actorUid: uid,
    actorRole: "user",
    createdAt: serverTimestamp(),
  });
