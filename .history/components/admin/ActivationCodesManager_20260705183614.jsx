        ...
        <section className="admin-editor">
          ...
          <div className="admin-user-section-grid">
            <section className="admin-user-section">
              ...
              >
                <strong style={{ color: "#0f172a" }}>Tao ma trong modal</strong>
                <span style={{ color: "#64748b", lineHeight: 1.7 }}>
                  De giu layout gon, thao tac tao ma le va tao loat ma da duoc chuyen vao modal. Danh sach ben trai va panel chi tiet ben phai se tap trung cho van hanh va doi soat.
                </span>
                <div className="admin-form-actions" style={{ margin: 0 }}>
                  <button type="button" onClick={() => { resetDraft(); setCreateMode("single"); setIsCreateModalOpen(true); }} disabled={saving}>
                    Tao ma le
                  </button>
                  <button type="button" className="admin-secondary-button" onClick={() => { resetDraft(); setCreateMode("bulk"); setIsCreateModalOpen(true); }} disabled={saving}>
                    Tao lo ma
                  </button>
                </div>
              </div>
          ...
          <section className="admin-user-section">
            <div className="admin-user-section-head">
              ...
              <span>{selectedEvents.length} event</span>
            </div>
            {!selectedCode ? (
              <EmptyState message="Chon mot code de xem audit timeline." />
            ) : loadingEvents ? (
              <EmptyState message="Dang tai audit timeline..." />
            ) : selectedEvents.length === 0 ? (
              <EmptyState message="Code nay chua co audit event nao." />
            ) : (
              <div className="admin-user-list-grid">
                {selectedEvents.map((item) => (
                  <article className="admin-user-list-card" key={item.id}>
                    <strong>{item.message || item.action || "Admin event"}</strong>
                    <span>{item.action || item.type || "event"}</span>
          ...
          <details className="admin-json-block">
            ...
            <pre>{JSON.stringify({ selectedCode, selectedUser, selectedEvents, stats, selectedCodeIds }, null, 2)}</pre>
          </details>
        </section>
      </div>
