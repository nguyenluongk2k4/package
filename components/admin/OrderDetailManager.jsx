...
function FieldStack({ label, children, noBorder = false }) {
  return (
    <div
      style={{
        display: "grid",
        gap: "6px",
        padding: "10px 0",
        borderBottom: noBorder ? "none" : "1px solid rgba(5, 52, 44, 0.06)",
      }}
    >
      <strong
        style={{
...
function FieldStackGroup({ children }) {
  return (
    <div
      style={{
        display: "grid",
...
export default function OrderDetailManager({ orderId }) {
  ...
  async function applyStatusUpdate(nextOrderStatus, options = {}) {
    ...
    try {
      ...
      if (nextOrderStatus === "paid" || payload.paymentStatus === "paid") payload.paidAt = serverTimestamp();
      ...
      if (nextOrderStatus === "failed" || payload.paymentStatus === "failed") payload.failedAt = serverTimestamp();
  ...
  return (
    <AdminLayout resource="orders">
      ...
      <div className="admin-toolbar" style={{ justifyContent: "space-between", flexWrap: "wrap" }}>
        <a href="/admin/orders" className="admin-btn-secondary" style={{ textDecoration: "none" }}>
          ← Quay lại danh sách
        </a>
        {order ? (
          ...
          <div style={{ display: "grid", gap: "20px" }}>
            ...
            <div className="admin-user-section-grid">
              <section className="admin-user-section">
                ...
                <FieldStackGroup>
                  <FieldStack label="Tên">{order.customer?.name || "Chưa có"}</FieldStack>
                  <FieldStack label="Email">{order.customer?.email || "Chưa có"}</FieldStack>
                  <FieldStack label="Số điện thoại">{order.customer?.phone || "Chưa có"}</FieldStack>
                  <FieldStack label="Tài khoản" noBorder>
                    {order.userId ? (
              ...
              <section className="admin-user-section">
                ...
                <FieldStackGroup>
                  <FieldStack label="Địa chỉ">{order.shippingAddress?.addressLine || "Chưa có"}</FieldStack>
                  <FieldStack label="Tỉnh / TP">{order.shippingAddress?.province || "Chưa có"}</FieldStack>
                  <FieldStack label="Quận / Huyện">{order.shippingAddress?.district || "Chưa có"}</FieldStack>
                  <FieldStack label="Phường / Xã" noBorder>{order.shippingAddress?.ward || "Chưa có"}</FieldStack>
                </FieldStackGroup>
              </section>
            </div>
            ...
            <div className="admin-user-section-grid">
                  ...
                  <label>
                    ...
                    <select value={draft?.paymentMethod || "cod"} onChange={(event) => setDraftField("paymentMethod", event.target.value)}>
                      {PAYMENT_METHOD_OPTIONS.map((item) => (
                        <option key={item} value={item}>{PAYMENT_METHOD_LABEL[item] || item.toUpperCase()}</option>
                      ))}
                    </select>
                  </label>
                  <label>
              ...
              <section className="admin-user-section">
                ...
                <FieldStackGroup>
                  <FieldStack label="Tạm tính">{formatVnd(order.subtotal)}</FieldStack>
                  <FieldStack label="Phí ship">{formatVnd(order.shippingFee)}</FieldStack>
                  <FieldStack label="Giảm giá">{formatVnd(order.discount)}</FieldStack>
                  <FieldStack label="Tổng tiền">{formatVnd(order.total)}</FieldStack>
                  <FieldStack label="Đã thanh toán lúc">{formatDate(order.paidAt)}</FieldStack>
                  <FieldStack label="Bắt đầu giao">{formatDate(order.shippingStartedAt)}</FieldStack>
                  <FieldStack label="Hoàn thành lúc">{formatDate(order.completedAt)}</FieldStack>
                  <FieldStack label="Hủy lúc" noBorder>{formatDate(order.cancelledAt)}</FieldStack>
                </FieldStackGroup>
