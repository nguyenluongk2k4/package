...
const ORDER_ACTION_BUTTON_STYLE = {
  minHeight: "40px",
  padding: "0 16px",
  ...
  return (
    <div className="admin-pagination">
      ...
      <div className="admin-pagination-actions">
        <button type="button" className="admin-btn-secondary" onClick={() => onChange(currentPage - 1)} disabled={currentPage === 1}>
          Trước
        </button>
        <span className="admin-pagination-page">
          Trang {currentPage} / {totalPages}
        </span>
        <button type="button" className="admin-btn-secondary" onClick={() => onChange(currentPage + 1)} disabled={currentPage === totalPages}>
          Sau
        </button>
      </div>
    </div>
...
export default function OrdersListManager() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, statusFilter, paymentMethodFilter, paymentStatusFilter, sortField, sortDir]);
  ...
  return (
    <AdminLayout resource="orders">
      <header className="admin-heading">
        ...
        <p>Xem danh sách đơn hàng theo kiểu bảng, tìm kiếm nhanh, sắp xếp, phân trang và đi sang trang chi tiết của từng đơn.</p>
      </header>
                ...
                <tbody>
                  {pagedOrders.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <button type="button" className="admin-table-action" style={ORDER_ACTION_BUTTON_STYLE} onClick={() => openOrderDetail(item.id)}>
                          Xem chi tiết
                        </button>
                      </td>
                    </tr>
