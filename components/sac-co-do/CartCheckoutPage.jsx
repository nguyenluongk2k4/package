          ...
          <aside className="heritage-cart-summary" aria-label="Tong don hang">
            ...
            <div className="heritage-cart-summary-panel">
              ...
              {items.length > 0 ? (
                ...
                >
                  {checkoutOpen ? "An form dat hang" : "Tien hanh dat hang"}
              ...
              {orderSuccess ? (
                <div
                  style={{
                    ...
                    padding: "18px",
                    borderRadius: "16px",
                    border: "1px solid rgba(16, 76, 39, 0.16)",
                ...
                >
                  <strong style={{ color: "#104c27", fontSize: "18px" }}>Dat hang thanh cong</strong>
                  ...
                  {draft.paymentMethod === "sepay" ? (
                    <a href={`/cua-toi?order=${orderSuccess.id}`} style={{ color: "#104c27", fontWeight: 800 }}>
                ...
                >
                  <div
                    style={{
                      padding: "16px",
                      borderRadius: "16px",
                      border: "1px solid rgba(16, 76, 39, 0.12)",
                      background: "#f8fafc",
                      display: "grid",
                      gap: "12px",
                    }}
                  >
                    <strong style={{ color: "#063823" }}>Thong tin dat hang</strong>

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      Ho va ten
                      <input value={draft.name} onChange={(event) => setDraftField("name", event.target.value)} placeholder="Nhap ten nguoi nhan" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                    </label>

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      Email
                      <input type="email" value={draft.email} onChange={(event) => setDraftField("email", event.target.value)} placeholder="email@example.com" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                    </label>

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      So dien thoai
                      <input value={draft.phone} onChange={(event) => setDraftField("phone", event.target.value)} placeholder="Nhap so dien thoai" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                    </label>

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      Dia chi chi tiet
                      <input value={draft.addressLine} onChange={(event) => setDraftField("addressLine", event.target.value)} placeholder="So nha, duong, khu vuc..." style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                    </label>

                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "10px" }}>
                      <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                        Ward
                        <input value={draft.ward} onChange={(event) => setDraftField("ward", event.target.value)} placeholder="Phuong / xa" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                      </label>
                      <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                        District
                        <input value={draft.district} onChange={(event) => setDraftField("district", event.target.value)} placeholder="Quan / huyen" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                      </label>
                      <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                        Province
                        <input value={draft.province} onChange={(event) => setDraftField("province", event.target.value)} placeholder="Tinh / thanh pho" style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px" }} />
                      </label>
                    </div>

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      Phuong thuc thanh toan
                      <select value={draft.paymentMethod} onChange={(event) => setDraftField("paymentMethod", event.target.value)} style={{ minHeight: "48px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "0 14px", background: "#ffffff" }}>
                        <option value="cod">COD - Thanh toan khi nhan hang</option>
                        <option value="sepay">SePay - Chuyen khoan</option>
                      </select>
                    </label>

                    {draft.paymentMethod === "sepay" ? (
                      <div style={{ padding: "12px 14px", borderRadius: "12px", background: "#eff6ff", color: "#1d4ed8", fontSize: "13px", lineHeight: 1.6 }}>
                        Don SePay se duoc tao truoc, sau do he thong se chuyen ban sang cong thanh toan SePay. Neu bi gian doan, ban co the mo lai trong trang Cua toi.
                      </div>
                    ) : null}

                    <label style={{ display: "grid", gap: "6px", fontWeight: 700, color: "#063823" }}>
                      Ghi chu
                      <textarea value={draft.notes} onChange={(event) => setDraftField("notes", event.target.value)} placeholder="Luu y giao hang, thoi gian nhan..." style={{ minHeight: "110px", borderRadius: "10px", border: "1px solid #cbd5e1", padding: "12px 14px", resize: "vertical" }} />
                    </label>
                  </div>
